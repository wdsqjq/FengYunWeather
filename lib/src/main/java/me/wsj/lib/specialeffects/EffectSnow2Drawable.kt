 ...
 * Rain
 */
class EffectSnow2Drawable(val type: Int, val rains: Array<Drawable>) : Drawable(), Animatable {
...
//            canvas.drawCircle(rain.x, rain.y, rain.radius, paint)
            ...
            canvas.save()
            canvas.rotate(
                (rain.degree / (2 * Math.PI)).toFloat(),
                drawable.bounds.centerX().toFloat(), drawable.bounds.centerY().toFloat()
            )
